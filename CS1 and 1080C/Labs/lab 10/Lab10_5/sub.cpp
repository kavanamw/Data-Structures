...
char sub::cipher(char letter, string cipher_key, string crypt){
    char ascii[26];
    int charl = 65;
    for(int x = 0; x < 26; x++){        // make ascii array the alphabet in caps
        ascii[x] = charl;
        charl++;
    }
    for(int i = 0; i < 26; i++){
...
string sub::encrypter(string line, string cipher_key){
    string newline = line;
...
string sub::decrypter(string line, string cipher_key){
    string newline = line;
    for(int i = 0; i < line.size(); i++){
        if(islower(line[i])){
            newline[i] = toupper(line[i]);
...
void sub::encrypter(string input_file, string encrypted_file, string cipher_key){
...
void sub::decrypter(string encrypted_file, string decrypted_file, string cipher_key){
